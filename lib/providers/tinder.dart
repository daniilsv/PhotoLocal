...
class TinderProvider with ChangeNotifier {
  ...
  List<TinderImage> tinderImages = [
    ...
    {"image": "https://sun9-13.userapi.com/Chj5zTNVIvmgwF7UZ3XLqz76o43FbPC0WpRoug/M0f-mhSovS8.jpg"},
    {"image": "https://sun9-50.userapi.com/c629107/v629107914/3f9de/TmUlsK04qxw.jpg"},
  ].map((item) => serializers.deserializeWith(TinderImage.serializer, item)).toList();
  int order = 0;
  ...
  answerTinder(int index, bool answer) {
    ...
    store.record("tinderImages").put(DataBase.db, {
      "tinderImages": tinderImages.map((_) => serializers.serializeWith(TinderImage.serializer, _)),
    });
    order++;
    notifyListeners();
  }
  ...
  clearTinders() {
    for (int i = 0; i < tinderImages.length; i++) tinderImages[i].rebuild((_) => _..answer = null);
    order = 0;
