...
//  double _minHeight;
  ...
  @override
  TinderSwapCardState createState() => TinderSwapCardState();
...
class TinderSwapCardState extends State<TinderSwapCard> with SingleTickerProviderStateMixin {
  ...
  Widget _buildCard(BuildContext context, int realIndex) {
      ...
      return Align(
        alignment: _animationController.status == AnimationStatus.forward
            ? frontCardAlign = CardAnimation.frontCardAlign(
                    _animationController, frontCardAlign, _cardAligns[widget._stackNum - 1], widget._swipeEdge)
                .value
            : frontCardAlign,
    ...
    return Align(
      alignment:
          _animationController.status == AnimationStatus.forward && (frontCardAlign.x > 3.0 || frontCardAlign.x < -3.0)
    ...
    cards.add(new SizedBox.expand(
      child: new GestureDetector(
        onPanUpdate: (DragUpdateDetails details) {
          setState(() {
            frontCardAlign = new Alignment(frontCardAlign.x + details.delta.dx * 20 / MediaQuery.of(context).size.width,
                frontCardAlign.y + details.delta.dy * 30 / MediaQuery.of(context).size.height);

            if (widget.swipeUpdateCallback != null) {
              widget.swipeUpdateCallback(details, frontCardAlign, false);
            }
          });
        },
        onPanEnd: (DragEndDetails details) {
          if (widget.swipeUpdateCallback != null) {
            widget.swipeUpdateCallback(null, frontCardAlign, true);
          }
          animateCards(0);
        },
      ),
  ...
  void initState() {
    ...
    _animationController.addStatusListener((AnimationStatus status) {
      ...
      if (status == AnimationStatus.completed) {
        if (frontCardAlign.x < widget._swipeEdge && frontCardAlign.x > -widget._swipeEdge) {
          ...
          if (widget.swipeCompleteCallback != null) {
            widget.swipeCompleteCallback(
                frontCardAlign.x < 0 ? CardSwipeOrientation.LEFT : CardSwipeOrientation.RIGHT, index);
          }
...
typedef CardDragUpdateCallback = void Function(DragUpdateDetails details, Alignment align, bool isEnd);

enum AmassOrientation { TOP, BOTTOM, LEFT, RIGHT }

class CardAnimation {
  static Animation<Alignment> frontCardAlign(
      AnimationController controller, Alignment beginAlign, Alignment baseAlign, double swipeEdge) {
    double endX, endY;

    if (TinderSwapCardState._trigger == 0) {
      endX = beginAlign.x > 0
    ...
    } else if (TinderSwapCardState._trigger == -1) {
  ...
  static Animation<Size> backCardSize(AnimationController controller, Size beginSize, Size endSize) {
    return new SizeTween(begin: beginSize, end: endSize)
        .animate(new CurvedAnimation(parent: controller, curve: Curves.easeOut));
  }
